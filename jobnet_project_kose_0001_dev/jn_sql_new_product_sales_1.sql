...
-- セットアイテムコード,アイテムコード,発売日,金額

-- new_item_tmp_table
-- 利用システムコード,発送日時,発送日時+30日の日付,アイテムコード(セットコード),注文タイプ,注文数,単体金額
-- アイテムコード,発売日,限定品区分,金額,注文タイプ

-- 上記を結合して以下のテーブル作成
-- 利用システムコード,発送日時,発送日時+30日の日付,アイテムコード,注文タイプ,発売日,金額

-- 上記をグルーピングして以下のテーブル作成
-- 利用システムコード,発送日時,既存品合計,新商品合計
...
WITH  set_item_separate_tmp_table AS(
  ...
  FROM kosedmp_prd_secure.segment_common_set_mst A
    LEFT JOIN kosedmp_prd_secure.segment_common_item_mst B
...
FROM
  ...
  LEFT JOIN kosedmp_prd_secure.segment_common_item_mst E
...
SELECT
  system_code,
  shipped_ymd,
  SUM(new_product_sales_amount) AS new_product_sales_amount,
  SUM(existing_product_sales_amount) AS existing_product_sales_amount
  FROM (
    ...
    SELECT
      ...
      CAST(
        SUM(CASE
              WHEN A.user_order_detail_type = 'RETURN_ORDER' AND A.hatubai_date >= A.newitem_limit_timestamp
                THEN A.amount_sku_ex_vat * -1
              WHEN A.user_order_detail_type = 'NORMAL_ORDER' AND A.hatubai_date >= A.newitem_limit_timestamp
                THEN A.amount_sku_ex_vat
              ELSE 0
            END
          )
        AS integer) AS new_product_sales_amount,
      CAST(
        SUM(CASE
              WHEN A.user_order_detail_type = 'RETURN_ORDER' AND A.hatubai_date < A.newitem_limit_timestamp
                THEN A.amount_sku_ex_vat * -1
        ...
        AS integer) AS existing_product_sales_amount

      FROM new_item_tmp_table A
      ...
      SELECT
        ...
        dt as shipped_ymd,
        0 as total_amount,
        0 as new_product_sales_amount,
        0 as existing_product_sales_amount
