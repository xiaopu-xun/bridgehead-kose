...
WITH store_code_with AS (
  ...
  FROM (
      ...
      FROM kosedmp_prd_secure.jill_store_mst mst
        LEFT JOIN kosedmp_prd_secure.store_mst_contrast con
...
order_with AS ( 
  ...
  FROM
    kosedmp_prd_secure.jill_segment_ec_kpad_order_detail OD 
    LEFT JOIN kosedmp_prd_secure.jill_segment_ec_kpad_order O 
    ...
    LEFT JOIN kosedmp_prd_secure.prefecture_mst p 
      ON p.name = O.state 
  WHERE
    O.shipped_timestamp <> '' 
    AND TD_TIME_RANGE( 
