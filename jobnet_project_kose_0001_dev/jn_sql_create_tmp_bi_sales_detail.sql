...
SELECT
	...
	CAST(SUBSTRING(CAST(DATE_ADD('MONTH', -1, CAST(y1.date_tmp AS TIMESTAMP)) AS VARCHAR), 1, 10) AS DATE) AS last_month, --先月
	CAST(SUBSTRING(CAST(DATE_ADD('YEAR', -1, CAST(y1.date_tmp AS TIMESTAMP)) AS VARCHAR), 1, 10) AS DATE) AS last_year, --去年
...
SELECT
	...
	COALESCE(t5.out_sales_amount, 0) AS lm_out_sales_amount, --出荷前月売上
	0 AS total_lm_out_sales_amount, --出荷前月売上（累計）
	COALESCE(t6.out_sales_amount, 0) AS ly_out_sales_amount, --出荷前年売上
	0 AS total_ly_out_sales_amount, --出荷前年売上（累計）
	COALESCE(t1.price_result, 0) AS price_result, --受注売上
	0 AS total_price_result, --受注売上（累計）
	COALESCE(t5.price_result, 0) AS lm_price_result, --受注前月売上
	0 AS total_lm_price_result, --受注前月売上（累計）
	COALESCE(t6.price_result, 0) AS ly_price_result, --受注前年売上
...
LEFT OUTER JOIN
	kosedmp_prd_secure.bi_segment_topic t2 --基準日と利用システムコードをキーにトピックと結合
ON
...
LEFT OUTER JOIN --基準日と利用システムコードをキーに目標金額テーブルと結合
	kosedmp_prd_secure.bi_segment_sales_target t3
ON
...
LEFT OUTER JOIN
	kosedmp_prd_secure.bi_segment_external_shipping_sales t4 --基準日と利用システムコードをキーに出荷売上外販テーブルと結合
ON
