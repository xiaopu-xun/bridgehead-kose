...
WITH first_buy_time AS (				--初期購買日時
	...
	FROM
		kosedmp_prd_secure.segment_probance_order
	...
	FROM
		kosedmp_prd_secure.segment_common_order
	GROUP BY
		customer_code_hash
	...
	SELECT
		...
		eim.category_nm
	FROM
		kosedmp_prd_secure.segment_probance_order_detail spod
	LEFT JOIN
		kosedmp_prd_secure.ecbeing_item_mst eim
	ON
	...
	FROM
		...
		SELECT
			...
			spo.order_code
		FROM
			kosedmp_prd_secure.segment_probance_order spo
		LEFT JOIN
			kosedmp_prd_secure.segment_common_after_regist scar
		ON
