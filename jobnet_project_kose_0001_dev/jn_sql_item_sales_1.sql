...
FROM
  kosedmp_prd_secure.item_allocation_number AS ian
  INNER JOIN (
    SELECT
      MAX(time) AS max_time,
      year
      FROM kosedmp_prd_secure.item_allocation_number
      GROUP BY
       year
    ) AS new_ian
...
JOIN (
    ...
    FROM
      kosedmp_prd_secure.jill_segment_common_order_detail scod
      LEFT JOIN kosedmp_prd_secure.jill_segment_common_order sco
      ON sco.order_code = scod.order_code

      LEFT JOIN kosedmp_prd_secure.segment_common_item_mst im
