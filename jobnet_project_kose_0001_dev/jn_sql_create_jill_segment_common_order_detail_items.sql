...
-- jn_job_jill_limititem_0001/limited_item_sales_1.sql を参考に、
-- jn_job_jill_segment_0002 の 「セット品分解版order_detail」 を作成。
...
WITH set_item_separate_with AS(
  ...
  FROM
    ...
    ( 
      ...
      FROM
        kosedmp_prd_secure.segment_common_set_mst A 
        INNER JOIN kosedmp_prd_secure.segment_common_item_mst B 
  ...
  UNION ALL
   ...
   ( 
    ...
    WHERE
      ...
      AND hinmoku_cd not in( select item_code from kosedmp_prd_secure.segment_common_set_mst) -- セットでないもの
    ORDER BY
      1, 2
  ) 
  ORDER BY
...
SELECT
  ...
  , A.filecreatedate
  , D.limited_kbn --限定品区分
FROM kosedmp_prd_secure.jill_segment_common_order_detail as A
