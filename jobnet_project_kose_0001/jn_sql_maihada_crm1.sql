...
WITH order_num AS (
  ...
  FROM
    segment_common_order o
    INNER JOIN (
      SELECT
        spod.order_code
      FROM
        segment_common_order_detail spod
...
first_trial AS (
  ...
  FROM
    ...
    INNER JOIN (
      ...
      FROM
        segment_common_order AS ord
        LEFT JOIN segment_common_order_detail AS ordd
          ON ord.order_code = ordd.order_code
        LEFT JOIN ecbeing_item_mst AS eim
          ON ordd.item_code = eim.item_cd
        LEFT JOIN segment_common_after_regist AS cus
    ...
    LEFT JOIN segment_common_order AS ord
...
( SELECT adid,mediaid,adgroup1id,adgroup2id
  FROM tmp_adebis_ads ads
  GROUP BY adid,mediaid,adgroup1id,adgroup2id
) ads
JOIN tmp_adebis_medias medias
 ON ads.mediaid = medias.mediaid
JOIN tmp_adebis_adgroup1s gp1
 ON ads.adgroup1id = gp1.adgroupid
JOIN tmp_adebis_adgroup2s gp2
