/* 重複と注文毎の最新レコードのみとする抽出サブクエリ（既存のjn_job_common_segment_0006_prd参照）*/
...
SELECT
  ...
  , CASE WHEN bso.system_code = 'E' THEN --- Eの中で、
    ...
    WHEN bso.system_code = 'B' THEN 'B' -- Bの人はBのまま
    WHEN bso.system_code = 'C' THEN 'C' -- Cの人はCのまま
    WHEN bso.system_code = 'F' THEN 'F' -- Fの人はFのまま
    WHEN bso.system_code = 'G' THEN 'G' -- Gの人はGのまま
    WHEN bso.system_code = 'J' THEN 'J' -- Jの人はJのまま
    WHEN bso.system_code = 'L' THEN 'L' -- Lの人はLのまま
