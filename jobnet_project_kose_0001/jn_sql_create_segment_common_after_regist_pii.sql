...
SELECT
    ...
    , newest_cust.city
    , newest_cust.address1
    , newest_cust.address2
    , newest_cust.tel
    , newest_cust.tel_memo
    , newest_cust.last_name
    , newest_cust.first_name
    , newest_cust.last_kana
    , newest_cust.first_kana
    , newest_cust.mail_pc
    , newest_cust.mail_mobile
FROM(
    SELECT
        dist_cust.*
      , CASE
    ...
    FROM(
        SELECT
            cust.*
        FROM 
            ${td.pii_database}.customer_pii AS cust
            INNER JOIN (
                SELECT
                    customer_code_hash
                  , filecreatedate
        ...
        WHERE 
          new_cust.rnk = 1 --会員の最新値データのみを抽出する
        ) AS dist_cust
    ) AS newest_cust
